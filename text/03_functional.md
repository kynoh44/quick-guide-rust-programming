...
이런 특징들을 가지는 함수형 언어들에 익숙해지면 프로그램의 구조를 적절하게 나누고 좋은 설계를 할 수 있는 지혜가 생깁니다. 순수 함수만 있는 코드는 항상 예측이 가능하므로 버그를 잡기도 쉬워지고, 한번 만든 함수를 여기저기에서 재사용 하기에도 좋습니다. 물론 단점도 있습니다. 순수 함수만을 사용하다보면 코드가 길어질 수밖에 없습니다. 또 반복문이라는게 존재할 수 없습니다. 반복문은 카운터 값을 가질 수밖에 없기때문에  상태가 변하지 않는 데이터가 아니기 때문입니다. 그래서 반복문대신에 재귀를 사용하는데 이게 익숙해지기가 매우 어렵습니다. 그리고 순수 함수들이 가져다 쓰기는 쉬워도 잘 조합해서 사용한다는 것이 쉽지 않습니다. 함수 하나하나를 아주 유연하게 만들어야하는데 쉽지 않는 일입니다.

함수형 언어의 특징 중 3가지만을 소개했는데도 이미 러스트는 순수한 함수형 언어가 될 수 없다는 것을 아셨을 것입니다. 하지만 최대한 함수형 언어와 비슷하게 프로그래밍을 할 수 있습니다. 그러면 함수형 언어들이 갖는 장점들을 활용하면서도 함수형 언어들의 단점을 극복할 수 있습니다. 이번 장에서는 러스트에서 도입한 함수형 언어들의 특징들이 무엇이 있는지 알아보겠습니다. 함수형 언어 기법들이 중요한 이유가 잘 활용하면 성능을 높일 수 있고, 코드가 단순해져서 버그를 방지하는데도 도움이 되기 때문입니다. 그리고 쓰레드나 async를 사용하는 비동기 프로그래밍에서 많이 사용되기 때문에 미리 잘 이해할 필요가 있습니다.

예를 들어 이터레이터나 map 메소드 등을 잘 활용한다면 성능도 높이고, 읽기 좋은 코드를 만들 수 있습니다. 물론 함수형 프로그래밍의 스타일에도 익숙해져야하고, 이터레이터나 map의 특징을 잘 이해해야합니다. 하지만 생각보다 어렵지 않으니 걱정할 필요는 없습니다.
